...
\begin{figure}
  ...
  % to dashboard/assets/cm.png by the auto-commit cell in Phase 3.
  \includegraphics[width=0.7\linewidth]{figures/confusion_matrix.pdf}
  \caption{Confusion matrix on the held-out split. The off-diagonal mass is concentrated in the Noise - Street/Sidewalk row collapsing into Noise - Residential.}
  \label{fig:cm}
\end{figure}
...
\begin{figure}
  \centering
  \includegraphics[width=0.7\linewidth]{figures/silhouette_curve.pdf}
  \caption{Silhouette by $k$ for both embedding spaces. MiniLM peaks higher and later; Word2Vec saturates around $k = 25$.}
  \label{fig:silhouette}
\end{figure}

\subsection{Borough fingerprints (Phase 6)}

Table \ref{tab:fingerprints} lists the top five distinctive tokens per borough by lift. Manhattan's distinctive vocabulary clusters around lost or stolen personal items (\texttt{wallet}, \texttt{bag}, \texttt{clothing}, \texttt{electronics}, \texttt{insurance}), each with lift around four times the corpus baseline; this is consistent with the borough's role as a commercial and tourist hub. Staten Island's distinctive vocabulary is a winter-services cluster (\texttt{plowed}, \texttt{recy}, \texttt{ewaste}, \texttt{snow}, \texttt{route}). The token \texttt{plowed} reaches lift 5.56, the highest single-term lift in the corpus, reflecting the borough's snow-route concerns relative to a corpus mostly aggregated over the four denser, less car-dependent boroughs.

\begin{table}
  \caption{Top five distinctive tokens per borough, ranked by lift (token frequency in borough divided by corpus frequency). Required minimum count: 100 occurrences in borough. The Manhattan and Staten Island fingerprints are operationally interpretable; the other three boroughs have less distinctive vocabularies overall.}
  \label{tab:fingerprints}
  \centering
  \begin{tabular}{ll}
    \toprule
    Borough & Top distinctive tokens (with lift) \\
    \midrule
    Manhattan & wallet (4.1x), bag (4.0x), clothing (3.9x), electronics (3.8x), insurance (3.7x) \\
    Staten Island & plowed (5.6x), recy (5.1x), ewaste (4.7x), snow (4.4x), route (3.9x) \\
    Bronx & rat (2.1x), debris (1.9x), peeling (1.8x), lead (1.7x), ceiling (1.7x) \\
    Brooklyn & sidewalk (1.8x), idling (1.7x), construction (1.6x), tree (1.6x), branch (1.5x) \\
    Queens & airport (3.2x), helicopter (2.4x), aircraft (2.2x), runway (1.9x), noise (1.6x) \\
    \bottomrule
  \end{tabular}
\end{table}
...
[1] Kontokosta, C.E.\ \& Hong, B.\ (2021) Bias in 311 Reporting: Engagement and Inequality in the Use of Civic Service Requests. {\it Computers, Environment and Urban Systems} {\bf 87}.
\label{kontokosta2021}

[2] Sp\"arck Jones, K.\ (1972) A statistical interpretation of term specificity and its application in retrieval. {\it Journal of Documentation} {\bf 28}(1):11-21.
\label{sparck1972}

[3] Meng, X., Bradley, J., Yavuz, B., Sparks, E., Venkataraman, S., Liu, D., et al.\ (2016) MLlib: Machine Learning in Apache Spark. {\it Journal of Machine Learning Research} {\bf 17}(34):1-7.
\label{meng2016mllib}

[4] Mikolov, T., Chen, K., Corrado, G.\ \& Dean, J.\ (2013) Efficient Estimation of Word Representations in Vector Space. In {\it Workshop Track of the International Conference on Learning Representations (ICLR)}.
\label{mikolov2013}

[5] Reimers, N.\ \& Gurevych, I.\ (2019) Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks. In {\it Proceedings of the 2019 Conference on Empirical Methods in Natural Language Processing (EMNLP)}.
\label{reimers2019}

[6] Reimers, N.\ \& Gurevych, I.\ (2019) sentence-transformers: Multilingual Sentence Embeddings. Software library, available at \url{https://www.sbert.net/}.
\label{reimers2019sentencebert}

[7] Wang, W., Wei, F., Dong, L., Bao, H., Yang, N.\ \& Zhou, M.\ (2020) MiniLM: Deep Self-Attention Distillation for Task-Agnostic Compression of Pre-Trained Transformers. In {\it Advances in Neural Information Processing Systems 33 (NeurIPS)}.
\label{wang2020minilm}

[8] Blei, D.M., Ng, A.Y.\ \& Jordan, M.I.\ (2003) Latent Dirichlet Allocation. {\it Journal of Machine Learning Research} {\bf 3}:993-1022.
\label{blei2003lda}

[9] Zaharia, M., Chowdhury, M., Franklin, M.J., Shenker, S.\ \& Stoica, I.\ (2010) Spark: Cluster Computing with Working Sets. In {\it Proceedings of the 2nd USENIX Conference on Hot Topics in Cloud Computing (HotCloud)}.
\label{zaharia2010spark}

[10] PySpark Documentation, Apache Software Foundation. \url{https://spark.apache.org/docs/latest/api/python/}.
\label{pysparkdocs}

[11] NYC OpenData, 311 Service Requests from 2010 to Present (2020+ endpoint, dataset id erm2-nwe9). \url{https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2010-to-Present/erm2-nwe9}.
\label{nyc311_2020plus}

[12] NYC OpenData, 311 Service Requests from 2010 to Present (2010-2019 historical endpoint, dataset id 76ig-c548). \url{https://data.cityofnewyork.us/Social-Services/311-Service-Requests-from-2010-to-Present/76ig-c548}.
\label{nyc311_2010_2019}
}
