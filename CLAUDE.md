...
- **2026-05-02** — Phase 10 PASSED. BERT MiniLM (sentence-transformers/all-MiniLM-L6-v2, 80MB, 384-dim) encoded 100K stratified docs in 9.3 sec on Tesla T4 (10,719 docs/sec). KMeans k=30 silhouette = 0.6881 (vs Word2Vec 0.5256, +0.1625). Probes "loud music" -> Loud Music/Party (0.69), "pothole" -> Pothole (0.78), "streetlight" -> Street Light Lamp Missing (0.65). "rat infestation in the building" got hijacked by ENTIRE BUILDING phrase frequency — honest demo finding. Bert clusters are purer single-category (cluster 0 = 100% Blocked Driveway), w2v clusters were richer cross-category. Different signals for different goals.
- **2026-05-02** — Phase 6 pivot+PASS. NYC mzpm-a6vd community-districts dataset returns 71 features with null geometry; other ID variants 404. Pivoted to 5-borough granularity using bundled geojson + already-present `borough` column (no spatial join). 1.59M rows aggregated. Manhattan wallet/bag/clothing/electronics signature (lift 4+) is the strongest fingerprint; Staten Island `plowed` lift 5.56 is suburban character. Brooklyn fingerprint is weak because its complaint distribution is the most "average" — itself a finding.

---

## 12. Status

**Current phase:** Phase 6 PASSED (2026-05-02). Pivoted from community districts (mzpm-a6vd dataset broken) to 5-borough granularity. 1.59M rows aggregated. **Best per-borough fingerprints:** Manhattan = wallet/bag/clothing/electronics/insurance/receipt (Consumer Complaint signature, lift 3.7-4.4); Staten Island = plowed/recy/ewaste/weed (suburban signature, lift 5.56 on `plowed` is highest in dataset); Bronx = refrigerator/ceiling/lease (apartment maintenance); Queens = junction/sunken/foundation/highway (infrastructure); Brooklyn = generic terms (most "average" borough). Phase 6 artifacts pushed to GitHub via auto-commit.
**Last touched:** 2026-05-02
**Next action (auto-mode):** Phase 7 Streamlit dashboard. Build with current artifacts (Phase 6 only); other tabs degrade gracefully and light up as user re-runs Phases 3/4v2/5/10 (each notebook now auto-pushes via its commit cell).

**Demo-day intellectual contrast** (use this for Q&A): Word2Vec on a 1.2K-word vocab found cross-category clusters (rat+food+construction = urban decay). BERT MiniLM with zero training improved silhouette by +0.16 but produces purer single-category clusters. Both are valid - Word2Vec for latent issue discovery, BERT for similarity-based retrieval.
